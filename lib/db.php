		...
		for ($i = 1, $argn = 0; $i < $len; $i++) {
			if ($query_template[$i] == '?') {
				...
				elseif ($query_template[$i+1] == '`' && $query_template[$i-1] == '`') {
					 ...
					 * removed by mysqli_real_escape_string()
					 */
					if (!is_string($arg_cur)) {
				...
				elseif (($query_template[$i+1] == ' ' || $query_template[$i+1] == "\n" ||
					...
					if (!is_string($arg_cur)) {
						...
						elseif ($arg_cur == null) {
							...
							throw new Exception('invalid variable data type given in query' .
								' at parameter ' . strval($argn+1));
						}
					}
					else {
						/**
						 ...
						 */
						throw new Exception('string variable not quoted in query parameter ' .
							strval($argn+1));
					}
				}
					...
					throw new Exception('parameter delimiter used incorrectly (occurance ' .
						strval($argn+1) . ')');
				}
			}
